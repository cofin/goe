...
def resolve_project_dir(project_dir: str | None) -> Path:
    """Resolve the project directory, defaulting to current working directory."""
...
def guess_target() -> str | None:
    """Guess Rust target based on host platform."""
...
def resolve_output_path(output: str | None, output_name: str | None, project_dir: Path, target: str) -> Path:
    """Resolve output path for bundled distribution."""
    if output:
...
def resolve_cache_dir(cache_dir: str | None, project_dir: Path) -> Path:
    """Resolve cache directory for downloads."""
    if cache_dir:
...
def resolve_python_root(extract_dir: Path) -> Path:
    """Resolve python root directory after extraction."""
...
def rust_string_literal(value: str) -> str:
    """Return safe Rust string literal for a given value."""
...
def render_install_dir_expression(install_dir: Path) -> str:
    """Render Rust expression for install directory."""
...
def patch_pyapp_install_dir(pyapp_dir: Path, install_dir: Path) -> None:
    """Patch PyApp to use custom default installation directory."""
...
def manage() -> None:
    """Management commands for bundler."""
...
@click.option("--output-name", help="Override output filename (stored under dist/)")
@click.option("--pyapp-dir", type=click.Path(path_type=Path), help="Path to PyApp checkout to patch install dir")
def build_bundle(
    target: str | None,
