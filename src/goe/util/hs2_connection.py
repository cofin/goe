...
def hs2_connect_using_options(opts, ldap=False, plain=False, kerberos=False, unsecured=False):
    ...
    if ldap_user and (ldap_password or ldap_password_file):
        return bool(ldap)
    if hive_user and auth_mech == "PLAIN":
        return bool(plain)
    if kerberos_service:
...
def hs2_connection(orchestration_config, host_override=None, port_override=None):
    ...
    if hs2_connect_using_options(optvars, ldap=True):
        ...
        if ldap_password_file:
            with open(ldap_password_file) as fh:
    ...
    if hs2_connect_using_options(optvars, plain=True):
        hive_user = os.environ.get("HIVE_SERVER_USER")
    ...
    if hs2_connect_using_options(optvars, kerberos=True):
...
def hs2_env_options(override=None):
    ...
    default_port = "10000" if os.environ.get("QUERY_ENGINE", "IMPALA").upper() == "HIVE" else "21050"
    ...
    options.use_ssl = True if os.environ.get("SSL_ACTIVE", "false").lower() == "true" else False
    ...
    elif "hiveserver2_auth_mechanism" not in vars(options):
