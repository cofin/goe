...
def apply_not_null_columns_csv(
    ...
    if not_null_columns_csv:
        ...
        return new_columns
    if not_null_propagation == NOT_NULL_PROPAGATION_NONE:
...
def not_null_columns_csv_to_not_null_columns(not_null_columns_csv: str, rdbms_column_names: list) -> list | str:
...
def validate_not_null_columns_exist(not_null_columns: list, rdbms_column_names: list):
    ...
    bad_cols = list(set([_.upper() for _ in not_null_columns]) - set([_.upper() for _ in rdbms_column_names]))
    if bad_cols:
